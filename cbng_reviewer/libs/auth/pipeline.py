...
def update_username_from_central_auth(backend, user, *args, **kwargs):
    ...
    if (
        backend.name == settings.SOCIAL_AUTH_BACKEND_NAME
        and (central_user := WikipediaReader().get_central_user(user_id=user.central_user_id))
        and user.username != central_user.username
    ):
